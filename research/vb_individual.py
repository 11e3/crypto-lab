"""Volatility Breakout (VB) 개별 자산 백테스트 (바이낸스 선물).

매수: BTC > MA20 AND high > target (target = open + (prev_high - prev_low) * k)
매도: close < MA5 -> 다음날 open에 청산

Usage:
    python research/vb_individual.py
    python research/vb_individual.py --k 0.6 --ma-filter 30
"""
...
TAKER_FEE = 0.0005
SLIPPAGE = 0.0003
COST_PER_LEG_DEFAULT = TAKER_FEE + SLIPPAGE


def load_data(interval: str = "1d") -> dict[str, pd.DataFrame]:
...
def backtest_symbol(
    ...
    Args:
        k: volatility breakout 계수
        ma_filter: BTC MA 필터 기간
        ma_exit: 청산 MA 기간
    """
    df = df.copy()
    btc = btc_df.copy()
...
def print_results(results: list[BacktestResult], params: dict) -> None:
    """전 심볼 결과 테이블."""
    print("\n" + "=" * 95)
    print(f"VB INDIVIDUAL BACKTEST (k={params['k']}, ma_filter={params['ma_filter']}, "
          f"ma_exit={params['ma_exit']})")
    print("=" * 95)
    ...
    # 포트폴리오 (equal weight across all symbols)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="VB individual asset backtest")
    ...
    parser.add_argument("--cost", type=float, default=COST_PER_LEG_DEFAULT,
                        help="Cost per leg (default 0.0008 = 0.08%%)")
    parser.add_argument("--sweep", action="store_true")
    parser.add_argument("--interval", default="1d")
    parser.add_argument("--no-plot", action="store_true")
    args = parser.parse_args()

    data = load_data(args.interval)

    if "BTCUSDT" not in data:
        print("BTCUSDT required for MA filter")
        return
    ...
    if not args.no_plot:
        ...
        plt.suptitle(f"VB k={args.k}, MA_filter={args.ma_filter}, MA_exit={args.ma_exit}", fontsize=13)
        plt.tight_layout()
