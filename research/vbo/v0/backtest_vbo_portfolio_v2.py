#!/usr/bin/env python3
"""Backtest VBO portfolio - V2: Exit without BTC filter.

Change: Exit condition uses only coin's MA5, not BTC MA20.
"""
...
def backtest_portfolio(symbols: list[str], start: str | None = None, end: str | None = None) -> dict:
    ...
    for date in all_dates:
        ...
        # === SELL LOGIC (V2: Only coin MA5 exit, no BTC filter) ===
        for symbol in symbols:
            if positions[symbol] > 0:
                row = prices[symbol]
                # V2 CHANGE: Exit only when coin's prev_close < prev_ma5
...
def main():
    ...
    print("=" * 80)
    print("V2: Exit with coin MA5 only (no BTC filter on exit)")
    print("=" * 80)

    for n in range(2, 6):
        print(f"\n{n}-COIN COMBINATIONS:")
        print("-" * 80)
        print(f"{'Combo':<20} {'CAGR':>10} {'MDD':>10} {'Sharpe':>10}")
        print("-" * 80)
        ...
        for combo in combinations(all_symbols, n):
            ...
            try:
                ...
                print(f"{r['symbols_str']:<20} {r['cagr']:>9.2f}% {r['mdd']:>9.2f}% {r['sharpe']:>10.2f}")
            except Exception as e:
                print(f"{'+'.join(symbols):<20} Error: {e}")
