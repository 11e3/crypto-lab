#!/usr/bin/env python3
"""Backtest VBO portfolio - V4: No BTC filter, add MA20 entry condition.

Entry: VBO breakout + prev_close > MA5 + prev_close > MA20
Exit: prev_close < MA5
"""
...
def backtest_portfolio(symbols: list[str], start: str | None = None, end: str | None = None) -> dict:
    ...
    for date in all_dates:
        ...
        valid = all(
            ...
            not pd.isna(prices[symbol]['prev_ma20'])
            for symbol in symbols
        )
        ...
        # === BUY: VBO + MA5 + MA20 (no BTC filter) ===
        buy_candidates = []
        for symbol in symbols:
            if positions[symbol] == 0:
                row = prices[symbol]
                buy_signal = (
                    ...
                    row['prev_close'] > row['prev_ma20']  # V4: added MA20 condition
                )
                if buy_signal:
    ...
    if len(negative_returns) > 0 and negative_returns.std() > 0:
        sortino = (daily_returns.mean() / negative_returns.std()) * np.sqrt(365)
    else:
        sortino = 0.0
...
def main():
    ...
    print("=" * 90)
    print("V4: No BTC filter, Entry adds MA20 (VBO + MA5 + MA20)")
    print("=" * 90)

    for n in range(2, 6):
        print(f"\n{n}-COIN COMBINATIONS:")
        print("-" * 90)
        print(f"{'Combo':<25} {'CAGR':>10} {'MDD':>10} {'Sharpe':>10} {'Sortino':>10}")
        print("-" * 90)
        ...
        for combo in combinations(all_symbols, n):
            ...
            try:
                ...
                print(f"{r['symbols_str']:<25} {r['cagr']:>9.2f}% {r['mdd']:>9.2f}% {r['sharpe']:>10.2f} {r['sortino']:>10.2f}")
