...
**계산 예시**:
```
어제: 시가 50M, 고가 52M, 저가 49M
K = (52M - 49M) / 52M = 3M / 52M = 5.77%

오늘 시가 51M이면:
Target = 51M + 3M × K 값 = 예상 상승가
```
...
**노이즈 비율 필터**
```
short_noise (5일) = K값들의 5일 이평
long_noise (10일) = K값들의 10일 이평

조건: short_noise < long_noise
의미: 최근 변동성이 장기 평균보다 낮음
      → 시장이 안정적 = 신호 신뢰도 높음
```
