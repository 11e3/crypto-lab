"""Binance candle data collector with incremental update support.

Fetches OHLCV data from Binance API and stores in parquet format.
...
from src.config.constants import BINANCE_API_RATE_LIMIT_DELAY, BINANCE_DATA_DIR, parquet_filename
from src.data.binance_fetch import fetch_all_binance_candles
...
class BinanceDataCollector:
    """Collects and manages Binance OHLCV candle data."""

    def __init__(self, data_dir: Path | None = None) -> None:
        """Initialize the data collector.

        Args:
            data_dir: Directory to store parquet files. Defaults to data/raw/binance/
        """
        self.data_dir = data_dir or BINANCE_DATA_DIR
        self.data_dir.mkdir(parents=True, exist_ok=True)

    def _get_parquet_path(self, ticker: str, interval: BinanceInterval) -> Path:
        """Generate parquet file path for given ticker and interval.

        Args:
            ticker: Binance symbol (e.g., 'BTCUSDT')
    ...
    def collect(
        ...
        # Fetch new candles
        new_df = fetch_all_binance_candles(ticker, interval, since=since)
    ...
    def collect_multiple(
        self,
        tickers: list[str],
        intervals: list[BinanceInterval],
        full_refresh: bool = False,
    ) -> dict[str, int]:
        """Collect data for multiple tickers and intervals.

        Args:
            tickers: List of Binance symbols
