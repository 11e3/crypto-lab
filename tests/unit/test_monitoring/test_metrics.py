...
class TestTradingMetrics:
    ...
    def test_record_order_non_filled_skips_volume(self) -> None:
        ...
        assert (
            metrics.order_volume_krw.labels(
                symbol="KRW-BTC",
                action="buy",
            )._value.get()
            == 0.0
...
class TestMLMetrics:
    ...
    def test_track_prediction_context_class_label_bug(self) -> None:
        ...
        metrics = MLMetrics()
        with (
            pytest.raises(ValueError, match="Incorrect label names"),
            metrics.track_prediction("regime_v1") as result,
        ):
...
class TestPipelineMetrics:
    ...
    def test_record_dag_run(self) -> None:
        ...
        assert (
            metrics.dag_runs_total.labels(
                dag_id="ohlcv_collect",
                state="success",
            )._value.get()
            == 1.0
        )
